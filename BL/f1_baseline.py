...
def evaluate_model(data_loader, model):
    ...
    with torch.no_grad():
        for inputs, labels in data_loader:
            ...
            # Calculate group labels based on predicted benefit
            predicted_benefits = preds.numpy()
...
def main_model_training():
    ...
    test_labels = torch.tensor(test_df[['benefit']].values, dtype=torch.float32)
