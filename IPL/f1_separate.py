...
class SingleTaskMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.sigmoid(self.fc4(x))  # Sigmoid를 사용하여 출력값을 [0, 1] 범위로 만듦
        return x
...
def evaluate_model(data_loader, model):
    ...
    accuracy = accuracy_score(all_actual_group_labels, all_predicted_group_labels)
    f1 = f1_score(all_actual_group_labels, all_predicted_group_labels, average='weighted')
